...
const Item: FC<ItemProps> = (props) => {
  ...
  return (
    <div className="flex justify-start items-center text-sm py-2 dark:text-white">
      <div className="w-1/6 flex-shrink-0">{label}</div>
      <div
        className={cx(
          "flex-grow transition-all",
          href ? "cursor-pointer hover:text-blue-500" : ""
        )}
        onClick={() => {
  ...
  return (
    <SettingContentLayout>
      ...
      <BaseCard title="关于">
        ...
        <Item
          label="参考"
          value="https://gitmoji.js.org/en-US/"
          href="https://gitmoji.js.org/en-US/"
        />
        <Item
          label="查找Emoji"
          value="https://www.emojiall.com/"
      ...
      <BaseCard title="项目依赖">
        {Object.entries(packageData.dependencies).map(
          ([key, value], j: number) => (
            <div
              className="flex justify-between items-center py-2 dark:text-white cursor-pointer hover:text-blue-500 transition-all text-sm"
              key={j}
              onClick={() => {
